...
class CaptchaController extends Controller {
	...
	public function __construct($assetsPath = null, $defaultImages = null, $defaultAudios = null ) {

        // If no assetsPath is specified, set the default
		if ( ! $assetsPath || empty( $assetsPath ) ) {
			$this->assetsPath = __DIR__ .'/visual-captcha';
		} else {
	...
	public function generate( $numberOfOptions = 5 ) {
		...
		return response()->json($this->getFrontendData());
	}
	...
	public function streamAudio() {
        ...
        if ( !empty( $audioFileName ) ) {
            ...
            return $this->utilStreamFile( $audioFilePath );
        }
    ...
    public function streamImage( $index, Request $request ) {
        ...
        $isRetina = intval( $request->input('retina') ) >= 1;
        ...
        if ( !empty( $imageFileName ) ) {
        	return $this->utilStreamFile( $imageFilePath );
        }
    ...
    private function utilStreamFile( $filePath ) 
    {
